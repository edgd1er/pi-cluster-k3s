...
def main():
    ...
    if main_partition_name != SD_CARD_PARTITION_NAME:
        module.exit_json(changed=False,
                         stderr="WARN: The root partition {} does not appear to be on an SD card, resize via raspi-config will not work. Doing nothing.".format(
                             main_partition_name))
    else:
        (rc, out, err) = module.run_command(["sudo", "raspi-config", "--expand-rootfs"], check_rc=True)

        fs_expanded = (lambda x, y: x - y)(
            *[int(l.split()[2]) for l in out.split("\n") if l.startswith(main_partition_name)])
